...
app.put('/updatePointDetails', (req, res) => {
    ...
    let query =
      ...
      Object.keys(detail)
        ...
        .join(",") +
      " where ScanID = ?";
    const parameters = [...Object.values(detail), req.query.ScanID];
    pool.query(query, parameters, function (err, results, fields) {
      if (err) throw err;

      if (results.affectedRows > 0) {
        return res.status(200).json({code:200, message: "updated scan details is successfully."});
      } else {
        return res.status(200).json({code:200, message: "update failed for scan details."});
      }
    });
})
...
app.put('/po', async (req, res) => {
    try {
        ...
        Promise.all([alreadyAvailableFilesPromise]).then(allData => {
           ...
           let updatePoInvoiceData = updatePOInvoice(poInvoiceDetails, req);
            Promise.all([updatePoWithoutImage, updatePoInvoiceData])
              .then((allData) => {
                return res
                  .status(200)
...
function updatePoWithoutImageData(detail,workOrderValues, req){
...
function updatePoDataWithImageData(detail, workOrderValues, req) {
...
function workOrderWoInvoicePromise() {
    return new Promise((resolve, reject) => {
        let query = `select * from WOInvoice
...
function insertWOInvoice(woInvoiceDetails, req, woId) {
    return new Promise((resolve, reject) => {
      ...
      for (var j = 0; j < woInvoiceDetails.length; j++) {
        var singleData = woInvoiceDetails[j];
        if (!!!singleData['WOInvoiceID']) {
            singleData.WOID = woId;
            var insertWoInvoicePromise = insertWoInvoiceData(singleData, woId);
    
            Promise.all([
              insertWoInvoicePromise,
            ...
            let sql = `SELECT 1 FROM WOInvoice WHERE WOInvoiceID = ${singleData['WOInvoiceID']}`
            pool.query(sql, function (err, result, fields) {
                if (err) throw err;
                if (result.length > 0) {
                    let query = `Update WOInvoice SET  ` + Object.keys(singleData).map(key => `${key}=?`).join(",") + " where WOInvoiceID = ?"
                    const parameters = [...Object.values(singleData), singleData['WOInvoiceID']]
                    pool.query(query, parameters, function (err, results, fields) {
                        if(err) throw err;
                        resolve(results);
                    })
...
function insertWoInvoiceData(singleData, woId) {
    return new Promise((resolve, reject) => {
        console.log("before---------> " + woId)

        console.log("inside insert WOInvoice");
        var sql =
            "INSERT INTO WOInvoice(WOID, Invoice) VALUES (?,?)";
        let parameters = [
            woId,
            singleData["Invoice"]
        ];
        pool.query(sql, parameters, function (err, result, fields) {
            if (err) throw err;
            if (result.affectedRows > 0) {
                resolve("updated WOInvoice");
            } else {
                resolve("update failed WOInvoice");
            }
        });
...
function updateWorkOrderPromiseData(req, workOrderWorkers, detail) {
    return new Promise((resolve, reject) => {
                        ...
                        if (result.length > 0) {
                            ...
                            pool.query(sql, parameters, function (err, result, fields) {
                                ...
                                if (result.affectedRows > 0) {
                                    ...
                                    resolve({ code: 401, "message": "New work order staff not updated." })
                                }
                            });
                        }
                    })
                    ...
                    if (result.length > 0) {
                        ...
                        pool.query(sql, function (err, result, fields) {
                            ...
                            if (result.length > 0) {
                                ...
                                resolve({ code: 200, "message": "No workers present to be removed." })
                            }
                        })
                    }
                })
...
function updateWOInvoice(woInvoiceDetails, req) {
    return new Promise((resolve, reject) => {
        if (!woInvoiceDetails || (woInvoiceDetails && woInvoiceDetails.length == 0)) {
            resolve("no findings to update");
            return;
        }
        let sql = `
        Delete FROM WOInvoice WHERE WOID = ${req.query.WorkOrderID}
        `;
        pool.query(sql, function (err, result, fields) {
            if (err) throw err;
            if (result) {
                ...
                for (var j = 0; j < woInvoiceDetails.length; j++) {
                    ...
                    var insertPoInvoicePromise = insertWoInvoiceData(singleData, req.query.WorkOrderID);


                    //it will run always to delete not in the list
                    var findingsIdList = [];
                    for (var i = 0; i < woInvoiceDetails.length; i++) {
                        findingsIdList.push(woInvoiceDetails[i].WOInvoiceID);
                    }
...
app.get('/scheduleReportWo', async (req, res) => {
    ...
    JOIN SiteZone ON WorkOrder.SiteZoneID = SiteZone.SiteZoneID
            WHERE WorkOrderStaff.StaffID = ${req.query.StaffID}
            and Date(WorkOrder.AssignedDateTime) BETWEEN NOW() + INTERVAL 1 DAY AND DATE_ADD(NOW(), INTERVAL 8 DAY)
            Order by DATE(WorkOrder.AssignedDateTime), Site.SiteName
            `
    pool.query(query, function (err, results) {
...
function updateWorkOrderWorkStatus(req) {
    ...
    return new Promise((resolve, reject) => {
      let query = `update WorkOrder set WorkStatusID = 6 , RescheduledReason = '${req.body.RescheduledReason}' where workorderId=${req.body.WorkOrderID}`;
      pool.query(query, function (err, results, fields) {
        if (err) throw err;
...
app.post('/saveScanDetails', async (req, res) => {
    let insertQuery = `insert into Scan_Details(UID, PointID, UserID, ScanDateTime) VALUES(?,?,?,?) `
    let parameters = [req.body.UID, req.body.PointID, req.body.UserID, req.body.ScanDateTime]
        pool.query(insertQuery, parameters, function (err, result) {
            if (err) throw err;
            if (result.affectedRows > 0) {
                return res.status(200).send({ code: 200, message: "inserted Scan_Details successfully" })
            } else {
                return res.status(400).send({ code: 400, message: "inserted Scan_Details failed" })
            }
          });
})
