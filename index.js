...
app.get('/scheduleReportWo', async (req, res) => {
    ...
    JOIN SiteZone ON WorkOrder.SiteZoneID = SiteZone.SiteZoneID
            WHERE WorkOrderStaff.StaffID = ${req.query.StaffID}
            and Date(WorkOrder.AssignedDateTime) BETWEEN NOW()+1 AND DATE_ADD(NOW()+1, INTERVAL 7 DAY)`
    pool.query(query, function (err, results) {
...
function updateWorkOrderWorkStatus(req) {
    ...
    return new Promise((resolve, reject) => {
      let query = `update WorkOrder set workStatusId = 6  and RescheduledReason = '${req.body.RescheduledReason}' where workorderId=${req.body.WorkOrderID}`;
      pool.query(query, function (err, results, fields) {
        if (err) throw err;
